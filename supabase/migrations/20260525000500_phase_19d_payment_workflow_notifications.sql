-- ─────────────────────────────────────────────────────────────────────────────
-- confirm_mock_payment_workflow_v1 — add PAYMENT_SUCCEEDED / PAYMENT_FAILED
-- notification after the mutation is applied and webhook event is marked processed
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- Step 8b: Notify booking owner of payment outcome
  perform private.create_notification_once_v1(
    v_booking.user_id,
    case when p_simulate_success
      then 'PAYMENT_SUCCEEDED'::public.notification_type
      else 'PAYMENT_FAILED'::public.notification_type
    end,
    case when p_simulate_success
    ...
    case when p_simulate_success
      then 'Your payment was successful. Your booking is now confirmed.'
    ...
    pg_catalog.jsonb_build_object(
      ...
      'payment_id', v_payment.id,
      'status',     case when p_simulate_success then 'SUCCEEDED' else 'FAILED' end
    ),
    v_payment.id::text || case when p_simulate_success then ':PAYMENT_SUCCEEDED' else ':PAYMENT_FAILED' end
  );
